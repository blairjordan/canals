...
function Popup({ children, timeoutDuration = undefined, onClose = () => {}, tabs = [] }) {
  ...
  const renderTabs = () => {
    ...
    return (
      <div className={'flex justify-start mb-4 border-b-2 border-gray-600'}>
        {tabs.map((tab, index) => (
          <div
            key={index}
  ...
  return (
    <div
      className={`transition-opacity duration-${FADEOUT_DURATION} ease-out ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
      <div className='px-10 py-8 text-sm shadow-xl rounded-lg bg-zinc-800 md:text-base z-50'>
        <div className='flex flex-col h-full' style={{ userSelect: 'none' }}>
          {renderTabs()}
          <div className='flex-1 tracking-wider'>{renderTabContent()}</div>
        </div>
      </div>
    </div>
  )
