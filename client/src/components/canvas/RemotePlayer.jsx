...
const RemotePlayer = ({ playerId = 0 }) => {
  ...
  useFrame((state, delta) => {
    updateRemotePlayerPosition(
      {
        x: helperRef.current.target.x,
        y: helperRef.current.target.y,
        z: helperRef.current.target.z,
        r: helperRef.current.rotation,
      },
      delta,
    )
  })
  ...
  return (
    ...
    remotePlayerRef.current && (
      <>
        <Boat ref={remotePlayerRef} playerId={playerId} isRemotePlayer={true} />
        <PlayerName position={remotePlayerRef.current.position} username={remotePlayerRef.username} />
