...
-- 🔑 user.provider_id is set by postgraphile
CREATE OR REPLACE FUNCTION current_player_id()
RETURNS BIGINT
SECURITY DEFINER
AS $$
  SELECT id FROM players WHERE provider_id = current_setting('user.provider_id');
$$ LANGUAGE sql STABLE;
...
BEGIN
  ...
  FROM player_markers(current_player_id(), 'fishing_spot', marker_distance_limit) gm
...
SELECT 1 FROM players WHERE id = current_player_id();
